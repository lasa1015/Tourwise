...
import { AccessTimeRounded, CloseRounded, DateRangeRounded, ExpandLessRounded, ExpandMoreRounded, LocationOnRounded } from '@mui/icons-material';
...
const EventCard_PopUp = ({ event, onClose }) => {
  ...
  const handleAdd = () => {
    ...
    addItemWithDateCheck(eventData, openMissingDatesAlert, 'EventCard_PopUp');
  };

  const isAdded = isItemInList(event.name);

  return (
    <Stack direction="row" sx={{ width: { xs: '310px', sm: '450px' } }}>
      {/* Close Button */}
      ...
      {/* Right Info Area */}
      <Box sx={{ marginLeft: { xs: '10px', sm: '20px' }, flexGrow: 1 }}>
        <Typography
          sx={{
        ...
        <Stack gap="4px" marginTop="2px">
          ...
          <Box display="flex" alignItems="center" marginTop={1}>
            ...
            <Typography variant="body2" color="text.secondary"
              sx={{ fontSize: { xs: '12px', sm: '14px' }, marginRight: { xs: '35px', sm: '50px' } }}>
              {date}
            </Typography>
          ...
          {isExpanded && (
            <Typography
              ...
              sx={{ marginLeft: '3px', width: { xs: '200px', sm: '300px' }, fontSize: { xs: '12px', sm: '14px' } }}>
              {event.description}
            </Typography>
          )}
        </Stack>
